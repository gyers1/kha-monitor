...
from application import CrawlService, get_scorer
...
def get_articles(
    ...
    result = []
    for a in articles:
        rel = scorer.score(a.title, a.category, a.site_name)
        result.append(
            ArticleResponse(
                id=a.id,
                site_id=a.site_id,
...
_SUMMARY_SELECTORS = [
    "#content", "#contents", ".board-view", ".board_view", "#board_view",
    ".view_con", ".view_cont", ".view-content", ".view_area", ".viewArea",
    ".bbs_view", ".bbs_content", ".bd_content", ".board_cont", ".board_txt",
    ".con_view", ".cont_area", ".cont_view", ".dbdata", ".table_view", ".tbl_view",
    ".board_stream", ".view_body", ".vw_cont", "article", ".content",
]


async def _fetch_summary(url: str) -> str:
    """기사 상세페이지에서 핵심 요약을 추출(외부 LLM 없이). 메타 설명 → 본문 첫 문단 순."""
    if not url:
        return ""
    low = url.lower()
    # 관보 등 본문이 PDF/파일 뷰어인 경우: 제목으로 충분하므로 추출 생략
    if "gwanbo.go.kr" in low or "ezpdf" in low or low.endswith(".pdf"):
        return ""
    try:
        async with httpx.AsyncClient(timeout=8, follow_redirects=True, verify=False,
                                     headers={"User-Agent": "Mozilla/5.0 (HousingMonitor)",
                                              "Accept-Language": "ko-KR,ko;q=0.9"}) as client:
            resp = await client.get(url)
        if resp.status_code != 200:
            return ""
        # 한글 정부 사이트는 EUC-KR/UTF-8 혼재 → 바이트로 넘겨 인코딩 자동 감지
        ...
        if len(desc) < 25:
            ...
            for sel in _SUMMARY_SELECTORS:
                ...
                if el:
                    ...
                    if len(best) > 120:
                        break
            if len(best) < 40 and soup.body:
                best = soup.body.get_text(" ", strip=True)
            if len(best) > len(desc):
                desc = best
        return _trim_summary(desc, 200)
    except Exception:
        return ""


@router.get("/summary")
async def article_summary(url: str = Query(..., description="기사 상세 URL"), db: Session = Depends(get_db)):
    """기사 주요내용 요약(외부 API 없이 상세페이지에서 추출). content_summary 컬럼에 캐시."""
    from infrastructure.database.models import ArticleModel
    art = db.query(ArticleModel).filter(ArticleModel.url == url).first()
    if art and (art.content_summary or "").strip():
        return {"summary": art.content_summary, "cached": True}
    summary = await _fetch_summary(url)
    if art and summary:
        art.content_summary = summary
        db.commit()
    return {"summary": summary, "cached": False}


@router.get("/summary-check")
async def summary_check(db: Session = Depends(get_db)):
    """출처별 요약 추출 가능 여부 진단. 각 활성 사이트의 최신 기사 1건으로 시도한다."""
    from infrastructure.database.models import SiteModel, ArticleModel
    sites = db.query(SiteModel).filter(SiteModel.is_active == True).all()  # noqa: E712
    targets = []
    for s in sites:
        art = (
            db.query(ArticleModel)
            .filter(ArticleModel.site_id == s.id)
    ...
    async def _try(url):
        if not url:
            return ""
        try:
            return await _fetch_summary(url)
        except Exception:
            return ""

    summaries = await asyncio.gather(*[_try(t["url"]) for t in targets])
    out = []
    for t, summ in zip(targets, summaries):
        summ = summ or ""
        if not t["url"]:
            status = "none"
        elif len(summ) >= 40:
            ...
            status = "fail"
        out.append({
            "site": t["site"], "category": t["category"], "url": t["url"],
            "title": t["title"], "chars": len(summ), "snippet": summ[:140], "status": status,
        })
    return out
