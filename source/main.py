...
from infrastructure.auth import is_session_ok, set_session_ok
...
async def morning_briefing():
    """매일 오전 9시: 오늘의 핵심(협회 공통 기준)을 묶음 요약 알림으로 보낸다."""
    SessionLocal = get_session_factory()
    session = SessionLocal()
    try:
        ...
        scored = []
        for a in articles:
            r = scorer.score(a.title, a.category, a.site_name)
            if r.is_core:
                scored.append((r.score, a))
        if not scored:
            print("[BRIEFING] 오늘 핵심 항목이 없어 아침 알림을 건너뜁니다.")
            return
        scored.sort(key=lambda x: x[0], reverse=True)
        top = [a for _, a in scored[:3]]
        notifier = get_notifier()
        await notifier.notify(top, subject=f"오늘의 핵심 {len(scored)}건")
        print(f"[BRIEFING] 아침 브리핑 알림 발송: 핵심 {len(scored)}건")
    except Exception as exc:
        print(f"[BRIEFING] 아침 브리핑 실패: {exc}")
    finally:
        session.close()
...
async def lifespan(app: FastAPI):
    ...
    scheduler.start()
    print('[SCHEDULE] Scheduler started: 1 min tick (category round-robin) + 09:00 morning briefing')
...
async def auth_guard(request: Request, call_next):
    # 인증은 구글 Apps Script(auth_script_url + auth_token)가 설정된 경우에만 강제한다.
    # 설정이 없으면(예: 사내 테스트) 게이트를 열어 앱이 그대로 동작한다.
    settings = get_settings()
    auth_enabled = bool(settings.auth_script_url and settings.auth_token)
    if not auth_enabled:
        set_session_ok(True)
    else:
...
def _render_template(name: str) -> HTMLResponse:
    settings = get_settings()
    template_path = Path(settings.resource_dir) / "presentation" / "templates" / name
...
async def home():
    """직원용 기본 화면 = 기존 대시보드(직원 모드). 경로(/)로 직원 모드 판별, 관리 기능 숨김 + 현안 강조."""
    return _render_template("dashboard.html")


@app.get("/admin", response_class=HTMLResponse)
async def dashboard():
    """관리자용 전체 대시보드(설정·사이트·크롤링 등). 숨겨진 진입점."""
    return _render_template("dashboard.html")


@app.get("/summary-check", response_class=HTMLResponse)
async def summary_check_page():
    """관리자용 요약 추출 진단 페이지."""
    return _render_template("summary_check.html")
